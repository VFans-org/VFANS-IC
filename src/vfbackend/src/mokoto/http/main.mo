...
public func counter(input:Nat) : async Nat { 
    if(input==1){
...
//This method sends a POST request to a URL with a free API we can test.
  ...
  public func test_get_http() : async Text {

    //1. DECLARE IC MANAGEMENT CANISTER  声明一个IC管理容器
    //We need this so we can use it to make the HTTP request   我们需要这个，这样我们就可以使用它来发出HTTP请求
    let ic : Types.IC = actor ("aaaaa-aa");

    //2. SETUP ARGUMENTS FOR HTTP GET request  为HTTP GET请求设置参数

    Cycles.add(20_949_972_000);
    let host : Text = "www.zhihu.com";
